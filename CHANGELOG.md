...
- `krowk sessions sync`, the incremental import meant to run on a schedule.
  ...
  watermark. A file that grew is read, one that shrank is re-read from 0 —
  the store's foreign-id dedup keeps that from inserting duplicates — and a
  transcript with no `import_state` row yet is new and always read. There is
