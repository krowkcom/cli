...
//   - .webm is audio/webm. Only a Matroska head with a video track makes it
//     video (contentTypeFor), so an audio-only recording stays audio.
//
// Anything absent is application/octet-stream; the card page falls back to the
// filename for those.
