...
// BenchmarkSessionList10k times the listing on the 10k-session fixture for
// -bench runs. The fixture builds once; each iteration pays Open plus the
// listing and close.
func BenchmarkSessionList10k(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		if len(rows) != perfSessionCount {
			db.Close()
			b.Fatalf("listed %d, want %d", len(rows), perfSessionCount)
		}
		if err := db.Close(); err != nil {
