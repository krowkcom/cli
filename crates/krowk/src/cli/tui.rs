...
//! flags it takes (`--model`, `--permission-mode`, `--resume [id]`), the
...
pub(super) fn run(ctx: &mut Ctx) -> Result<(), Error> {
    ...
    let host = HostConfig { sessions_dir, cwd, registry, krowk_version: super::VERSION.into(), pricer: prompt::pricer(ctx.io.env) };
